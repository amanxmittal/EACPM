...
| Grid | `ux4g-row`, `ux4g-col-12`, `-col-sm-6`, `-col-lg-3`/`-4` | About, Contact, Media, Publications (was `.grid-2/3/4`) |
| Utilities | `ux4g-mt/mb/my/mx/m-*`, `-py/px/pt/pb-*`, `-d-flex/-grid/-block`, `-ai-*`, `-jc-*`, `-flex-none`, `-inline-gap-*`, `-place-items-center` | ~50 one-off `style={{}}` props across every page |
...
**Grid was adopted with the breakpoint model deliberately changed (user decision, 2026-07-30):** UX4G's grid is mobile-first (`ux4g-col-sm-*` ≥576px, `ux4g-col-lg-*` ≥992px) where ours was desktop-first (max-width 620/960px) — every `.grid-2/3/4` site now collapses at different exact viewport widths than before. Two real bugs surfaced while wiring this up, both now fixed and worth knowing before touching a grid again:
- **Never put a `ux4g-col-*` class on the same element as `.card` (or any element with its own `padding`).** The vendor's gutter is a padding-then-cancel-with-negative-margin trick — `ux4g-row>*` sets `padding-inline`/`padding-block`, and `ux4g-row` cancels it with a matching negative margin so the *columns* tile edge-to-edge while their *padding* creates the visual gap. `.card`'s own `padding` shorthand has equal specificity and loads later, so it silently wins and zeroes out the gutter's padding — the result is cards touching border-to-border with no gap, invisible at a glance because each card's own internal padding still insets its *text*, just not its *border*. Always wrap: `<div className="ux4g-col-*"><div className="card">…</div></div>`, never `<div className="ux4g-col-* card">`. Verified via `getBoundingClientRect()`, not by eye — this one looks fine in a casual screenshot.
- **`--ux4g-gutter-y` is not pre-halved the way `--ux4g-gutter-x` is.** `padding-inline: calc(gutter-x * .5)` splits the horizontal gutter so two neighbours' halves recombine to the full value — but `padding-block: gutter-y` (no `* .5`) applies the *full* value to both the top and bottom of every item, so two vertically-stacked items' paddings sum to 2×. `.grid` pins `--ux4g-gutter-y` to *half* the target gap to compensate; `--ux4g-gutter-x` stays unhalved. Get this backwards and every wrapped row sits twice as far from the one above it as the columns are from each other.

Both were caught by measuring `getBoundingClientRect()` gaps directly, not by screenshot review — a doubled or zeroed gutter is easy to miss by eye at a quick glance, especially against a dotted background.

**Inline `style={{}}` sweep (user decision, 2026-07-30: full sweep, round to nearest token).** Of ~122 inline styles found across 16 files, ~25 in the 3 chart components are computed SVG geometry/colour (`stroke: s.color`, `x={x(i)}`) and can never become static classes — left inline, out of scope. Of the remaining ~95, everything mapping to margin/padding/gap/display/flex/align/justify was converted; `maxWidth`, `height`, `aspectRatio`, `fontSize`, `transform`, `cursor`, `borderRadius`, `boxShadow`, `width`, `gridTemplateColumns` and any `clamp()`/`scrollMarginTop` value were left inline as out of scope (sizing/typography/motion, or a value that is fluid or a computed header-offset by design, not a flat magic number). Where no exact token matched a rem value, rounded to the nearest available step on the `2xs/xs/s/m/l/xl/2xl` scale; the two 3rem instances (both meant as generous outer spacing, not fine-grained rhythm) had no nearby step at all — the utility scale tops out at `2xl` = 32px — and rounded down to it anyway per the chosen approach, a visible ~33% reduction. Three real gotchas surfaced, all fixed:
- **No font-weight utility exists.** `ux4g-fw-semibold` and siblings only set `--ux4g-fw-current` — a custom property the shared type-ramp rule reads, but only on elements carrying `ux4g-heading-*`/`-body-*`/`-label-*`/etc. On a bare `<span>` or `<div>`, the class is completely inert: text silently renders at inherited weight, not semibold. Every `fontWeight` inline style in the codebase stays inline; there is no fix, only avoidance.
- **`ux4g-d-flex` has 4 decoy redeclarations elsewhere in the vendor CSS** (e.g. `.ux4g-result-list-v2 ... .ux4g-d-flex{display:contents!important}`, scoped under a component we don't use) alongside the one real, unscoped utility (`.ux4g-d-flex{display:flex!important}`). A naive first-match string search finds a decoy; the extractor's proper brace-parsing pulls all 5 rules, but only the unscoped one ever matches our markup — the other 4 are dead selectors, harmless. Verify a utility's *body*, not just that a selector search returned something.
- **`.cluster`'s own CSS sets `align-items: center`** (its default), which beats `ux4g-ai-start` the same way `.card`'s padding beat the grid gutter — equal specificity, `.cluster` loads later. `ux4g-jc-*` has no such conflict (`.cluster` never sets `justify-content`), so it converts cleanly; `align-items` overrides on a `.cluster` element have to stay inline. Choose by whether the class you're pairing with already has an opinion on that property, not by whether a matching utility exists.

Every conversion (display, gap, align/justify, margin/padding tiers, the risky `.card`-adjacent button padding) was verified with `getComputedStyle`/`getBoundingClientRect`, not screenshots alone — three of the gotchas above are exactly the kind of thing that renders "close enough" to pass a casual visual check.
